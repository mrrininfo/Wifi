    ...
    def __handle_wpas(self, pixiemode=False, verbose=None):
        ...
        if line.startswith('WPS: '):
            if 'Building Message M' in line:
                ...
                print('[*] Hack Injecting In Router M{}…'.format(n))
            elif 'Partially Done M' in line:
                n = int(line.split('Received M')[1])
                self.connection_status.last_m_message = n
                print('[*] Hacking Security Keys M{}'.format(n))
                if n == 5:
...
class WiFiScanner:
    ...
    def prompt_network(self) -> str:
        ...
        if not networks:
            print('[-] No Valid Target Found Boss.')
            return
        while 1:
            try:
...
def usage():
    return """
OneShotPin 0.0.2 (c) 2017 rofl0r, modded by D4RK H4CK3R
